...
test.describe("account-claim-booking", () => {
  ...
  test.describe('P0-01 Signup never links a guest booking by email alone', () => {
    test.setTimeout(120_000);
    ...
    test('signup with the booking email claims nothing', async ({ page }) => {
      const email = uniqueEmail('no-auto-claim');
      const seeded = await seedLinkedBooking({
        carId,
        status: 'confirmed',
        pickupDate: range.pickupDate,
