...
describe('purgeExpired open physical rental retention', () => {
  ...
  test('extends overlapping blocks for open rentals before deleting expired ones (all cars)', async () => {
    await purgeExpired();

    expect(clientQuery).toHaveBeenCalledTimes(2);

    const [extendCall, deleteCall] = clientQuery.mock.calls;
  ...
  test('extends then deletes for a single car', async () => {
  ...
  test('continues to guarded delete when extend hits a GiST overlap', async () => {
    ...
    expect(clientQuery).toHaveBeenCalledTimes(2);
    expect(clientQuery.mock.calls[1][1]).toMatch(/NOT EXISTS/);
  });
