-- Phase 11: Admin Fleet Calendar
-- Extend car_date_blocks + calendar_tasks + calendar RBAC permissions
...
-- Existing rows are booking-synced ranges
UPDATE car_date_blocks SET block_type = 'booking' WHERE block_type IS NULL OR block_type = 'booking';
...
CREATE TABLE IF NOT EXISTS calendar_tasks (
  ...
  CONSTRAINT calendar_tasks_valid_type CHECK (task_type IN (
    'delivery', 'pickup', 'return_assist', 'cleaning', 'inspection', 'custom'
  )),
...
-- owner + manager: all calendar permissions
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
CROSS JOIN permissions p
WHERE r.slug IN ('owner', 'manager')
  AND p.key LIKE 'can_%calendar%'
