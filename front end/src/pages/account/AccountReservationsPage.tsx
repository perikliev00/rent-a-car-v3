...
export function AccountReservationsPage() {
  ...
  return (
    <div className="mx-auto min-w-0 max-w-5xl overflow-x-clip px-4 py-10 sm:px-6">
      ...
      {query.isError ? (
        ...
        <ul className="mt-8 divide-y divide-[var(--color-line)] border-y border-[var(--color-line)]">
          {reservations.map((r) => (
            <li key={r.id} className="flex min-w-0 flex-col gap-3 py-5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
              <div className="min-w-0">
              ...
              <Link to={`/account/reservations/${r.id}`} className="shrink-0">
                <Button size="sm" className="w-full sm:w-auto">Details</Button>
              </Link>
            </li>
          ))}
        </ul>
