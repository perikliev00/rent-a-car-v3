...
| --- | --- | --- |
| `npm test` | `test:unit` then `test:frontend` then `test:admin-frontend` | No |
...
| `npm --prefix "front end" run test:coverage` | Customer frontend Vitest with coverage | No |
| `npm --prefix admin-front-end run test:coverage` | Admin frontend Vitest with coverage | No |
...
| E2E | `e2e/specs/` | Real UI flows (customer search → checkout; admin staff UI) | API/DB-only checks with no page |
...
CI runs each group once (unit backend, customer frontend check, admin frontend check, db+integration+e2e). Seven former Playwright API/DB specs now live under `backend/tests/integration/`. E2E is ~24 domain suites. `workers: 1` is required (shared DB).
